...
test.describe("Ivey Profile Page Tests", () => {
  test.beforeEach(async ({ page }) => {
    await page.goto("https://pinaspeaks.com/mountaineer/@Ivey");
  });

  test("Verify the logo", async ({ page }) => {
    const PinasPeakslogo = page.getByRole("link", { name: "PinasPeaks Logo" });
    await expect(PinasPeakslogo).toBeVisible();
    await expect(PinasPeakslogo).toHaveCount(1);
    await expect(page).toHaveScreenshot({ path: "screenshots/profile-page_logo_visible_pass.png"});
  ...
  test("Verify the Order by dropdown by Elevation is working", async ({
    page,
  }) => {
    await test.step("Select the Order by Elevation (masl) in the dropdown", async () => {
      ...
      const resultMasl = page
        .locator("div")
        .filter({ hasText: "Mt. Pulag2926 MASL Ifugao," })
        .first();
      await expect(resultMasl).toBeVisible();
      // await page.screenshot({
      //   path: "screenshots/profile-page_order-by_masl-visible_pass.png",
      // });
  ...
  test("Verify the Order by dropdown by Name is working", async ({ page }) => {
    await test.step("Select the Order by Name in the dropdown", async () => {
      ...
      const resultName = page
        .locator("div")
        .filter({ hasText: "Malvar's Peak419 MASL Rizal" })
        .first();
      await expect(resultName).toBeVisible();
      // await page.screenshot({
      //   path: "screenshots/profile-page_order-by-name-visible_pass.png",
      // });
      await resultName.click();
    });
  });

  test("Verify the Order by dropdown by Date Climbed is working", async ({
    page,
  }) => {
    await test.step("Select the Order by Date Climbed in the dropdown", async () => {
      ...
      const resultDate = page
        .locator("div")
        .filter({ hasText: "Mt. Kapayas783 MASL Cebu Day" })
        .first();
      await expect(resultDate).toBeVisible();
      // await page.screenshot({
      //   path: "screenshots/profile-page_order-by-date_climbed-visible_pass.png",
      // });
  ...
  test("Verify the Search textbox", async ({ page }) => {
    await test.step("Search with valid input", async () => {
      ...
      // await page.screenshot({
      //   path: "screenshots/profile-page_search-valid-input_visible_pass.png",
      // });
    });

    await test.step("Search with invalid input", async () => {
      const searchBox = page.getByRole("textbox", { name: "Search..." });
      await expect(searchBox).toBeVisible();
